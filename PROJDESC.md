...
(eg: `python examine.py testsuite/extern/runs_extern.json` or `python examine.py runs_pngtest.png`)
This command
...
- the extensions for generated files are passed to the parser as an argument by initialize.py and examine.py
...
1. initialize.py: takes the project as input
	...
	- generate_static_info():
		- for each C/C++ file, say projects/exp/src/A.c, in dependencies.p, we know that a .o will be generated corr. to it. We
			- run parsers/clang_parser.py on it
				- that generates 3 files: A_clang.xml, A.calls, A.funcargs (explained later)
				- copies these 3 files to the folder location: outputs/exp/src/A.c/
			- run parsers/dwxml.py on A.o to generate A_dd.xml
			- run parsers/ddx.py with OFFSET flag
				- that combines only the local variables present in A.c from A_clang.xml and A_dd.xml
				- creates A_comb.xml and A.offset in outputs/exp/src/A.c/ (explained later)
...
3. examine.py: takes the executable as input
