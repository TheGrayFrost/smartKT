...
* For future development, feel free to contact
*/
...
// This key is used to separate the BasicBlock information from Call Graph information
// sections when printed and stored in output
std::string key("St5D7yHzyE5WRpHRGuGVB6r4t4HK47TRS69Gka7Pfc2d2wArVrmyPtUZbEUxVMrZ");

// This function returns the location of a BasicBlock. Implementation details, no algorithm
...
// This function takes the location string generated by the above function
// and returns in (line, col) pair
...
/*
This function is used to calculate the extent of a BasicBlock. Internally it uses
the above 2 functions to determine the start line, start col and end line, end col
for each basicBlocks. Remember, each basic Block has an ENTRY and an EXIT block, which
are special and fictional (they donot carry any code)
*/
std::string getExtentString(const CFGBlock &CB, SourceManager &SM, const CFGBlock &Entry, const CFGBlock &Exit){
	...
	if(&CB == &Entry) {
		...
		/*
		* Algorithm to find extent of each basicBlock:
		* 1. For each Element in the basic Block, get (start_line, start_col) and
		*    (end_line, endCol) pairs, and store them in a vector.
		* 2. Sort the vector using custom comparator
		* 3. The first pair in sorted array is the beginning and the last pair is
		*    the end of the basicBlock
		*/
		...
		sort(rangeLocs.begin(), rangeLocs.end(), [](const std::pair<int, int> &lhs, const std::pair<int, int> &rhs){
			// Custom comparator: compare on line and then on columns
		...
		// convert the extent to string format for output
...
/*
	This function returns the predecesors and successors of a BasicBlock
	in the format described in README. special care of ENTRY and EXIT
*/
...
/*
	Implement by inheriting and override the CallGraph class of Clang, to implement
	a printing function that helps us print in required format. The format of the
	print is described in README. Implementation details.
*/
			...
			if (f->hasBody()) {
				...
				*/
				...
				/*
					...
					This commented code, will help us determine Liveness information
					and embed the same in SmartKT. Contact for further discussions.
				*/
				// llvm::outs() << "-----------------------------------------------------" << '\n';
