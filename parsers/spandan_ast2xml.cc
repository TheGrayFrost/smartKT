...
CXChildVisitResult
visitor(CXCursor cursor, CXCursor parentCursor, CXClientData clientData) {
  ...
  std::string parentNodeId = std::to_string(clang_hashCursor(parentCursor));
  xmlNewProp(cur_ptr, BAD_CAST "id",
    BAD_CAST nodeid.c_str());
  TTLstring += "id:"+nodeid+ " relation:isA " + "object:" + std::string(clang_getCString(kindName)) + " .\n";
  TTLstring += "id:"+nodeid+ " relation:ast_parent " + "id:" + parentNodeId + " .\n";
  TTLstring += "id:"+parentNodeId + " relation:ast_child " + "id:" + nodeid + " .\n";
  ...
  // set cursor spelling
  {
    ...
    if( std::strlen(spelling_cstr) > 0 ){
      ...
      TTLstring += "id:"+nodeid+ " attribute:spelling \"" + std::string(clang_getCString(spelling_cstr)) + "\" .\n";
    }
  ...
  // if( ! clang_equalLocations(parentData->cur_loc, location) ) {
  {
    ...
    // if( ! clang_File_isEqual(par_file, file) ) {
    {
      ...
      TTLstring += "id:"+nodeid+ " attribute:file \"" + std::string(clang_getCString(fileName)) + "\" .\n";
      clang_disposeString( fileName );
    }
    xmlNewProp(cur_ptr, BAD_CAST "line",
      BAD_CAST std::to_string(line).c_str());
    TTLstring += "id:"+nodeid+ " attribute:line " + std::to_string(line) + " .\n";
  ...
  // Add cursor extent.
  {
    ...
    TTLstring += "id:"+nodeid+ " attribute:range_start \"" + st + "\" .\n";
    ...
    TTLstring += "id:"+nodeid+ " attribute:range_end \"" + en + "\" .\n";
  }
  ...
  // Cursor type info
  {
    ...
    if( std::strlen(type_spelling_cstr) > 0 ) {
      ...
      TTLstring += "id:"+nodeid+ " property:type \"" + std::string(type_spelling_cstr) + "\" .\n";
      /*
  ...
  { // Add USR if non empty
    ...
    TTLstring += "id:"+nodeid+ " attribute:usr usr:\"" + std::string(usr_cstr) + "\" .\n";
  ...
  { // Add lexical / semantic parents
    ...
    if( !clang_Cursor_isNull(lexicalParent) ) {
      ...
      if( std::strlen(usr_cstr) > 0 ){
        ...
        TTLstring += "id:"+nodeid+ " relation:lex_parent_usr usr:\"" + std::string(usr_cstr) + "\" .\n";
        TTLstring += "usr:\""+std::string(usr_cstr)+ "\" relation:lex_child_id id:" + nodeid + " .\n";
      }
      clang_disposeString( usr );
    }
    ...
    if( (!clang_Cursor_isNull(semanticParent)) &&
      ...
      if( std::strlen(usr_cstr) > 0 ){
        ...
        TTLstring += "id:"+nodeid+ " relation:sem_parent_usr usr:\"" + std::string(usr_cstr) + "\" .\n";
        TTLstring += "usr:\""+std::string(usr_cstr)+ "\" relation:sem_child_id id:" + nodeid + " .\n";
      }
      clang_disposeString( usr );
    }
  }
...

  {
    ...
    if( (!clang_Cursor_isNull(refc)) &&
      ...
      if( std::strlen(usr_cstr) > 0 ){
        ...
        TTLstring += "id:"+nodeid+ " relation:ref_usr usr:\"" + std::string(usr_cstr) + "\" .\n";
      }
      clang_disposeString( usr );
    }
   // Get definition cursor
      ...
      if( std::strlen(usr_cstr) > 0 ){
        ...
        TTLstring += "id:"+nodeid+ " relation:def_usr usr:\"" + std::string(usr_cstr) + "\" .\n";
      }
      clang_disposeString( usr );
      }
    }
  ...
  if( clang_isDeclaration( cursorKind ) ) {
    ...
    if( ac_spec_str != nullptr ) {
      ...
      TTLstring += "id:"+nodeid+ " property:access_specifier \""+ std::string(ac_spec_str) +"\" . \n";
    }
    ...
    if( linkage_kind != CXLinkage_Invalid ) {
      ...
      TTLstring += "id:"+nodeid+ " property:linkage_kind \""+ std::string(linkage_kind_str) +"\" . \n";
    }
  }
